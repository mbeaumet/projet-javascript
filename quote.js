...
const place_quote = document.querySelector("#citation p");
const name = document.querySelector("#auteur p");
let last = citations[0];
let nbrand = Math.floor(Math.random() * citations.length);
console.log(citations[2][1]);


document.querySelector("button").addEventListener("click",()=>{
  count =0;
  while (count < 1){
    place_quote.textContent = citations[nbrand];
    console.log(citations[nbrand]);
    document.querySelector("#auteur p").textContent = citations[nbrand][1];
    count++;
  }

  nbrand = Math.floor(Math.random() * citations.length);
  last = citations[nbrand];
  count++;
})
